...
| **P5 finding: Java bridge rejects Proxy wrapping** | ✅ | Android's `@JavascriptInterface` dispatch validates the injected object identity; a JS `Proxy` over `AndroidInterface` fails every native call with `Java bridge method can't be invoked on a non-injected object`. Fix: `webhost.js` implements the *full* bridge surface itself (typed `fwd()` forwarders for native methods, JS implementations for camera/record) and `waitForInterface` binds the shim directly — no Proxy. |
| **P6 finding: native WAL ≥ desktop .bak** | ✅ | Desktop rotates a whole-file `.bak` because sql.js exports the DB as one buffer. Native SQLite with WAL (enabled in `DbOpenHelper.onConfigure`) gives per-transaction durability — strictly stronger for kill/crash recovery. No redundant DB backup added; media files keep `.bak` rotation (P3). Verified by `am kill` → relaunch → 5/5 projects intact. |
| **P7 finding: tablet layout sound as-is** | ✅ | Salvage CSS (70ea661) fully handles 1280×800 tablet: lobby thumbs 278×208 (≥48dp targets), editor stage 482×362 centered, palette full-width, zero horizontal overflow, system bars hidden (insets [0,0]), `ImmersiveModeConfirmation` is a per-install debug-device prompt (suppress via `settings put secure immersive_mode_confirmations confirmed`). Phone-portrait stays landscape-locked (`sensorLandscape`) per non-goals. |
| **P8 finding: .sjr intent content URI permissions & round-trip** | ✅ | On Android 11+ (API 30+), external storage DocumentsProvider URIs (`content://com.android.externalstorage.documents/...`) require document picker grants; internal / `FileProvider` content URIs read cleanly via `ContentResolver.openInputStream`. Intent import queues payload in `MainActivity.pendingSjrBase64`, flushes upon lobby load (`document.body[data-scratchjr-page="home"]`), calls `PlatformBridge.loadProjectFromSjr`, and unzips + inserts gift project into SQLite (`isgift: 1`), verified live on emulator with rendered gift ribbon and thumbnail in lobby. |
| **P8 finding: native share sheet integration** | ✅ | `sendSjrUsingShareDialog` and `sendExportedPng` write to `cacheDir/exports/` and invoke `Intent.createChooser` with `FileProvider` URI + `FLAG_GRANT_READ_URI_PERMISSION`, properly focusing `com.android.intentresolver/.ChooserActivityLauncher` on API 36. |
| **P10 finding: release signing configuration** | ✅ | Configured release `signingConfig` in `android/app/build.gradle.kts` to pull keystore file and credentials from environment variables (`ANDROID_KEYSTORE_FILE`, `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS`, `ANDROID_KEY_PASSWORD`) without hardcoding secrets in repository. Builds unsigned/signed APK and AAB cleanly in both debug and release configurations. |
| **Phone Responsive Scaling finding: sub-740px viewport heights** | ✅ | Modern handheld phones in landscape with high DPI (e.g. 420 DPI on 1080p screens) produce CSS viewport heights of only ~360–412px. The legacy hardcoded `minStageHeight = 434px` and vertical balancing previously pushed the block palette and script workspace completely below the screen edge. Implemented `applyResponsiveFrameScale()` in `lib.ts` and scaled `Events.getTargetPoint` coordinates so phone viewports dynamically scale `#frame`, `#libframe`, and `#paintframe` to virtual 740px height — keeping the stage, block palette, and script coding area 100% visible, fully interactive, and touch-accurate on all phones without regressing desktop/tablet layout. |
