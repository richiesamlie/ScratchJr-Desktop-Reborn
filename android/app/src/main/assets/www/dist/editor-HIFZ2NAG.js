import {
  ...
  setEnginePorts,
  setModelRef
} from "./chunk-QCIDYJNY.js";
import {
  ScratchAudio,
  iOS
} from "./chunk-ACLAJ3J4.js";
import "./chunk-VS23VAOM.js";
import {
  gn,
  hit3DRect,
  isTouch,
...
var Scripts = class {
  ...
  scriptsMouseDown(e) {
    ...
    var target = e.target;
    if (target.nodeName == "H3" && getModelRefAs(target, "blockarg") === ScratchJr.activeFocus) {
      return;
    }
    ...
    for (var i = sc.childElementCount - 1; i > -1; i--) {
      ...
      var thsBlock = getModelRefAs(ths, "block");
      if (!thsBlock) {
        continue;
      }
      if (thsBlock.isCaret) {
  ...
  addBlockToScripts(b, dx, dy) {
    ...
    getModelRefAs(b, "block").moveBlock(dx, dy);
    for (var i = 1; i < this.dragList.length; i++) {
      var piece = this.dragList[i].div;
      piece.parentNode.removeChild(piece);
      this.sc.appendChild(piece);
    }
    this.layout(getModelRefAs(b, "block"));
    this.snapToPlace(this.dragList);
    if (getModelRefAs(b, "block").cShape) {
      this.sendToBack(getModelRefAs(b, "block"));
    }
    this.dragList = [];
  }
  sendToBack(b) {
  ...
  getBlocks() {
    ...
    for (var i = 0; i < sc.childElementCount; i++) {
      var b = getModelRefAs(sc.childNodes[i], "block");
      if (!b) {
        continue;
      }
      if (b.type != "block") {
...
setEnginePorts({
  ...
  projectRecreateObject: (page, name, data, callBack, active) => Project.recreateObject(page, name, data, callBack, active),
...
window.scratchjr.onExportProjectRequest(() => {
  try {
    ...
    if (!ref) return;
    void import("./IO-GKXSIRV3.js").then(({ default: IO }) => {
      IO.zipProject(ref, (contents) => {
        let name = "";
        try {
          name = document.forms.projectname.myproject.value || "";
        } catch (_) {
        }
...
//# sourceMappingURL=editor-HIFZ2NAG.js.map
