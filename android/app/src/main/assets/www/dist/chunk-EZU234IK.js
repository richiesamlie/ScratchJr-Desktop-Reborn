  ...
  */
  static getTargetPoint(e) {
    ...
    if (te && te.touches && te.touches.length > 0) {
      return {
        x: te.touches[0].pageX !== void 0 ? te.touches[0].pageX : te.touches[0].clientX,
        y: te.touches[0].pageY !== void 0 ? te.touches[0].pageY : te.touches[0].clientY
      };
    }
    if (te && te.changedTouches && te.changedTouches.length > 0) {
      return {
        x: te.changedTouches[0].pageX !== void 0 ? te.changedTouches[0].pageX : te.changedTouches[0].clientX,
        y: te.changedTouches[0].pageY !== void 0 ? te.changedTouches[0].pageY : te.changedTouches[0].clientY
      };
    }
    const me = e;
    if (me) {
      return {
        x: me.pageX !== void 0 ? me.pageX : me.clientX,
        y: me.pageY !== void 0 ? me.pageY : me.clientY
      };
    }
    return { x: 0, y: 0 };
  }
...
var PaintAction = class _PaintAction {
  ...
  static lineMouseDown() {
    currentShape = SVGTools.addLine(gn("layer1"), Paint.initialPoint.x, Paint.initialPoint.y);
  }
  static starMouseDown() {
    currentShape = SVGTools.addStar(gn("layer1"), Paint.initialPoint.x, Paint.initialPoint.y);
  }
  ...
  static starMouseMove(evt) {
    ...
    var rOuter = Math.hypot(delta2.x, delta2.y);
  ...
  static lineMouseUp(evt) {
    var box = SVGTools.getBox(currentShape);
    if (SVGTools.notValidBox(box)) {
      _PaintAction.removeShape(evt);
    }
  }
  static starMouseUp(evt) {
    var box = SVGTools.getBox(currentShape);
    if (SVGTools.notValidBox(box)) {
      _PaintAction.removeShape(evt);
    }
...
var Camera = class _Camera {
  ...
  static doAction(str) {
    switch (str) {
      case "cameraflip":
        ...
        PlatformBridge.choosecamera(view, _Camera.flip);
        break;
      case "camerasnap":
        _Camera.snapShot();
  ...
  static snapShot() {
    PlatformBridge.captureimage("Camera.processimage");
  }
  ...
  static processimage(str) {
    ...
    if (str != "error getting a still") {
      SVGImage.addCameraFill(target2, str);
    }
    _Camera.close();
    Paint.cameraToolsOff();
    Paint.selectButton("select");
...
var SVGTools = class _SVGTools {
  ...
  static getStarPath(cx, cy, rOuter, rInner) {
    var cmds = [];
    var numPoints = 5;
  ...
  static addStar(div, x, y) {
    var shape = document.createElementNS(Paint.xmlns, "path");
    var attr = {
      "id": getIdFor("path"),
      "opacity": 1,
      "d": _SVGTools.getStarPath(x, y, 1, 0.4)
    };
...
var Paint = class _Paint {
  ...
  static rightPalette(div) {
    ...
    _Paint.addSidePalette(rightpal, "filltools", PlatformBridge.camera == "1" && Camera.available ? ["camera", "paintbucket"] : ["paintbucket"]);
  }
  ...
  static initSprite(ow, oh) {
    ...
    namedForms.spriteform.name.value = gn(currentName) ? getModelRefAs(gn(currentName), "sprite").name : currentName;
    if (ow) {
      workspaceWidth = ow;
    }
    if (oh) {
...
var Prims = class _Prims {
  ...
  static applyToAllStrips(list, fcn) {
    ...
    for (var i = 0; i < page.div.childElementCount; i++) {
      var spr = getModelRefAs(page.div.childNodes[i], "sprite");
      if (!spr) {
        continue;
      }
...
var BlockArg = class {
  ...
  closePictureMenu(e, mu, b, c) {
    ...
    if (block.arg.argValue != value) {
      ...
      var action = {
        action: "scripts",
        where: getModelRefAs(spr.div.parentNode, "page").id,
        who: spr.id
      };
...
var ScriptsPane = class _ScriptsPane {
  ...
  static setActiveScript(sprname) {
    ...
    ScratchJr.stage.currentPage.setCurrentSprite(getModelRefAs(gn(sprname), "sprite"));
  ...
  static blockFeedback(dx, dy, e) {
    ...
    switch (Palette.getLandingPlace(script.dragList[0].div, e)) {
      case "library":
        ...
        if (thumb && getModelRefAs(gn(getModelRefAs(thumb, "spritethumb")), "sprite").type == ScratchJr.getSprite().type) {
  ...
  static dropBlock(e, el) {
    ...
    switch (Palette.getLandingPlace(el, e)) {
      ...
      case "library":
        ...
        if (thumb && getModelRefAs(gn(getModelRefAs(thumb, "spritethumb")), "sprite").type == getModelRefAs(gn(page.currentSpriteName), "sprite").type) {
...
var Record = class _Record {
  ...
  static record(e) {
    ...
    function doRecord() {
      if (isRecording) {
        ...
        PlatformBridge.sndrecord(_Record.startRecording);
      }
    }
  }
  static startRecording(filename) {
    if (parseInt(filename) < 0) {
      ...
      var poll = function() {
        PlatformBridge.volume(_Record.updateVolume, _Record.recordError);
      };
  ...
  static startPlaying() {
    PlatformBridge.startplay(_Record.timeOutPlay);
  ...
  static closeContinueSave() {
    PlatformBridge.recorddisappear("YES", _Record.registerProjectSound);
  }
  static closeContinueRemove() {
    PlatformBridge.recorddisappear("NO", _Record.tearDownRecorder);
  }
  static registerProjectSound() {
    function whenDone(snd) {
      if (snd != "error") {
        var spr = ScratchJr.getSprite();
        var page = getModelRefAs(spr.div.parentNode, "page");
...
var Palette = class _Palette {
  ...
  static dropBlockFromPalette(e, element) {
    ...
    switch (_Palette.getLandingPlace(element, e)) {
      case "scripts":
        PlatformBridge.analyticsEvent("editor", "new_block", getModelRefAs(element, "block").blocktype);
        var sc = ScratchJr.getActiveScript();
        ...
        const parentPage = getModelRefAs(spr.div.parentNode, "page");
        Undo.record({
          action: "scripts",
          where: parentPage.id,
          who: spr.id
...
var Sprite = class {
  ...
  touchingAny() {
    ...
    for (var i = 0; i < page.childElementCount; i++) {
      var other = getModelRefAs(page.childNodes[i], "sprite");
      if (!other) {
        continue;
      }
      if (other.type == "text") {
      ...
      if (Events.dragthumbnail && other == getModelRefAs(Events.dragthumbnail, "sprite")) {
        continue;
      }
  ...
  openBalloon(label) {
    ...
    const stageOwner = getModelRefAs(gn("stage"), "stage");
  ...
  unfocusText() {
    ...
    if (this.noChars(form.typing.value)) {
      ...
      if (changed2) {
        const parentPage = getModelRefAs(this.div.parentNode, "page");
...
var Page = class {
  ...
  setBackground(name2, fcn) {
    ...
    var me = this;
    var keys = MediaLib.keys;
    var url = keys[name2] ? MediaLib.path + name2 : name2.indexOf("/") < 0 ? PlatformBridge.path + name2 : name2;
  ...
  setPageThumb(c) {
    ...
    for (var i = 0; i < this.div.childElementCount; i++) {
      var spr = getModelRefAs(this.div.childNodes[i], "sprite");
      if (!spr) {
        continue;
      }
  ...
  encodePage() {
    ...
    data.num = this.num;
    const owner = this.currentSpriteName ? getModelRefAs(gn(this.currentSpriteName), "sprite") : null;
  ...
  getSprites() {
    ...
    for (var i = 0; i < spritelist.length; i++) {
      const owner = getModelRefAs(gn(spritelist[i]), "sprite");
  ...
  spriteAdded(spr) {
    var me = getModelRefAs(spr.div.parentNode, "page");
  ...
  modifySprite(md5, cid, sid) {
    var sprite = getModelRefAs(gn(unescape(sid)), "sprite");
    if (!sprite) {
...
var Grid = class _Grid {
  ...
  static updateCursor() {
    ...
    var obj = getModelRefAs(spr, "sprite");
    var c = gn("circlenum");
    if (!c) {
      return;
    }
...
var Stage = class _Stage {
  ...
  resetPage(obj) {
    var page = obj.div;
    for (var i = 0; i < page.childElementCount; i++) {
      var spr = getModelRefAs(page.childNodes[i], "sprite");
      if (!spr) {
        continue;
      }
      if (spr.type == "sprite") {
  ...
  copySprite(el, thumb) {
    ...
    var data = enginePorts().projectEncodeSprite(getModelRefAs(el, "pagethumb"));
    if (getModelRefAs(gn(getModelRefAs(thumb, "pagethumb")), "page") == this.currentPage) {
    ...
    var page = getModelRefAs(gn(getModelRefAs(thumb, "pagethumb")), "page");
    ...
    var whenDone = function(spr) {
      ...
      enginePorts().thumbsUpdateSprites();
      enginePorts().thumbsUpdatePages();
      const ownerPage = getModelRefAs(gn(getModelRefAs(thumb, "pagethumb")), "page");
      enginePorts().undoRecord({
        action: "copy",
        who: name2,
        where: ownerPage.id
  ...
  deletePage(str, data) {
    ...
    newp[catid] = sprAttr;
    newp.layers = [catid];
    var page = getModelRefAs(gn(str), "page");
    var indx = this.getPagesID().indexOf(str);
    if (indx < 0) {
      return;
    }
    var form = namedForms3.activetextbox;
  ...
  mouseDown(e) {
    ...
    if (namedForms3.activetextbox.textsprite) {
      return;
    }
    ...
    if (shaking3 && target3.id == "deletesprite") {
      this.removeSprite(getModelRefAs(shaking3, "sprite"));
      return;
    }
    ctx.clearRect(0, 0, 480, 360);
    ...
    if (shaking3 && hitobj && hitobj.id == shaking3.id) {
      ...
      const sprnameOwner = getModelRefAs(gn(sprname), "sprite");
  ...
  whoIsIt(ctx, pt) {
    ...
    for (var i = page.childElementCount - 1; i > -1; i--) {
      spr = getModelRefAs(page.childNodes[i], "sprite");
      if (!spr) {
        continue;
      }
      if (!spr.shown) {
    ...
    for (var j = page.childElementCount - 1; j > -1; j--) {
      spr = getModelRefAs(page.childNodes[j], "sprite");
      if (!spr) {
        continue;
      }
      if (!spr.shown) {
  ...
  startShaking(b) {
    ...
    getModelRefAs(b, "sprite").startShaking();
  }
  stopShaking(b) {
    if (!hasModelRef(b)) {
      return;
    }
    getModelRefAs(b, "sprite").stopShaking();
    enginePorts().setShaking(void 0);
    enginePorts().setStopShaking(void 0);
  }
  startSpriteDrag(e) {
    var spr = getModelRefAs(Events.dragthumbnail, "sprite");
  ...
  removeFromPage(spr) {
    ...
    if (sprite && sprite.id == spr.id) {
      ...
      page.setCurrentSprite(sprites.length > 0 ? getModelRefAs(gn(sprites[0]), "sprite") : void 0);
    }
  }
  ...
  clickOnElement(e, spr) {
    const owner = getModelRefAs(spr, "sprite");
    if (owner.type == "text") {
      if (!enginePorts().isInFullscreen()) {
        owner.clickOnText(e);
      }
...
var Library = class _Library {
  static init() {
    ...
    libFrame.ondragover = function(e) {
      e.preventDefault();
    };
    libFrame.ondrop = function(e) {
      e.preventDefault();
      if (e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files.length > 0) {
        _Library.handleImportFile(e.dataTransfer.files[0]);
      }
    };
  ...
  static layoutHeader() {
    ...
    importme.onmousedown = _Library.triggerImport;
    ...
    fileInput.onchange = function(e) {
      ...
      if (target3 && target3.files && target3.files.length > 0) {
        _Library.handleImportFile(target3.files[0]);
        target3.value = "";
      }
    };
  ...
  static triggerImport(e) {
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
    var input = gn("library_file_input");
    if (input) {
      input.click();
    }
  }
  static sanitizeImportName(fileName) {
  ...
  static handleImportFile(file) {
    ...
    var assetName = _Library.sanitizeImportName(file.name);
    ...
    if (ext === "svg") {
      reader.onload = function() {
        ...
        PlatformBridge.setmedia(svgBase64, "svg", function(svgMd5) {
      ...
      reader.onload = function() {
        ...
        img.onload = function() {
          ...
          PlatformBridge.setmedia(svgBase64, "svg", function(svgMd5) {
            PlatformBridge.setmedia(pngThumbBase64, "png", function(pngMd5) {
              var key = isCostume ? "usershapes" : "userbkgs";
              var row = isCostume ? {
                scale: 0.5,
                md5: svgMd5,
  ...
  static removeAssetFromLib(str) {
    ...
    if (data.altmd5) {
      _Library.assetThumbnailUnique(data.altmd5, type, function(isUnique) {
        if (isUnique) {
          PlatformBridge.remove(data.altmd5, PlatformBridge.trace);
        }
      });
    }
    IO.deleteobject(key, data.id, PlatformBridge.trace);
  }
...
var UI = class _UI {
  ...
  static addProjectInfo() {
    ...
    if (window.Settings.shareEnabled) {
      ...
      PlatformBridge.deviceName(function(name2) {
    ...
    function saveAndShare() {
      ...
      Project.prepareToSave(ScratchJr.currentProject, function() {
        ...
        IO.zipProject(ScratchJr.currentProject, function(contents) {
          ...
          PlatformBridge.sendSjrToShareDialog(IO.zipFileName, emailSubject, Localization.localize("SHARING_EMAIL_TEXT"), shareType, contents);
  ...
  static handleTextFieldSave(dontHide) {
    if (ScratchJr.isEditable() && ScratchJr.editmode == "storyStarter" && !Project.error) {
      PlatformBridge.analyticsEvent("samples", "story_starter_edited", Project.metadata.name);
  ...
  static finishTextFieldSave(dontHide) {
    ...
    PlatformBridge.setfield(PlatformBridge.database, Project.metadata.id, "name", pname);
    if (!dontHide) {
      ScratchAudio.sndFX("exittap.wav");
  ...
  static showInfoBox(e) {
    ...
    projectNameTextInput.onblur = function() {
      if (ScratchJr.isEditable()) {
        namedForms4.projectname.myproject.focus();
      }
    };
    ...
    if (ScratchJr.isEditable()) {
      namedForms4.projectname.myproject.value = Project.metadata.name;
    } else {
  ...
  static hideInfoBox(e, dontHide) {
    ...
    if (ScratchJr.isEditable()) {
      namedForms4.projectname.myproject.blur();
  ...
  static topLevelColor(e) {
    ...
    if (gn("fontcolorbutton").className == "changecolorText on") {
      ...
      gn("fontsizebutton").className = "fontsizeText off";
      gn("textfontsizes").className = "textuifont off";
      var text = namedForms4.activetextbox.textsprite;
  ...
  static setTextColor(e) {
    ...
    text.setColor(c);
    const textOwnerPage = getModelRefAs(text.div.parentNode, "page");
    Undo.record({
      action: "edittext",
    ...
    var ti = namedForms4.activetextbox.typing;
    ti.style.color = c;
  }
  ...
  static setTextSize(e) {
    ...
    setProps(namedForms4.activetextbox.style, {
...
var Thumbs = class _Thumbs {
  ...
  static getPageOrder() {
    ...
    while (page) {
      ...
      if (pagename) {
        res.push(getModelRefAs(gn(pagename), "page"));
      }
      page = page.next;
    }
    return res;
  ...
  static clickOnPage(e, pagename) {
    ...
    var page = getModelRefAs(gn(pagename), "page");
  ...
  static updateSprites() {
    ...
    for (var i = 0; i < sprites.length; i++) {
      ...
      var spr = getModelRefAs(s, "sprite");
      if (spr.type != "sprite") {
        continue;
      }
  ...
  static startDragThumb(e, tb) {
    if (ScratchJr.shaking && e.target.id == "deletespritethumb") {
      ScratchJr.clearSelection();
      ScratchJr.stage.removeSprite(getModelRefAs(gn(getModelRefAs(tb, "spritethumb")), "sprite"));
    }
    if (ScratchJr.shaking) {
      ScratchJr.clearSelection();
    }
  ...
  static clickOnSprite(e, el) {
    if (ScratchJr.shaking && ScratchJr.shaking == el) {
      ScratchJr.clearSelection();
      ScratchJr.stage.removeSprite(getModelRefAs(gn(getModelRefAs(el, "spritethumb")), "sprite"));
      return;
    }
    ...
    ScratchJr.unfocus(e);
    var spr = getModelRefAs(gn(spritename), "sprite");
    var page = getModelRefAs(spr.div.parentNode, "page");
  ...
  static prepareToDrag(e) {
    ...
    var spr = getModelRefAs(gn(getModelRefAs(Events.dragthumbnail, "spritethumb")), "sprite");
  ...
  static dragging(e, el) {
    ...
    for (var i = 0; i < gn("pagecc").childElementCount; i++) {
      ...
      if (thumb && thumb.id != spr.id) {
        const dragPage = getModelRefAs(page, "page");
  ...
  static removePagesCaret() {
    for (var i = 0; i < gn("pagecc").childElementCount; i++) {
      ...
      var page = gn(getModelRefAs(spr, "pagethumb"));
      const pageOwner = getModelRefAs(page, "page");
  ...
  static unhighlighSprite(spr) {
    ...
    for (var i = 0; i < currentsc.childElementCount; i++) {
      if (hasModelRef(currentsc.childNodes[i])) {
...
var Undo = class _Undo {
  ...
  static smartRecreate(cmd, elem, data) {
    ...
    switch (action) {
      case "pageorder":
        ...
        ScratchJr.stage.setPage(getModelRefAs(gn(data.currentPage), "page"), false);
        if (Palette.numcat == 5) {
          Palette.selectCategory(5);
        }
        break;
      case "changepage":
        ...
        break;
      case "changebkg":
        getModelRefAs(gn(page), "page").redoChangeBkg(data);
        break;
      case "scripts":
        _Undo.redoScripts(data, page, spr);
        if (spr && gn(spr)) {
          const pageOwner = getModelRefAs(gn(page), "page");
          pageOwner.setCurrentSprite(getModelRefAs(gn(spr), "sprite"));
          Thumbs.selectThisSprite(getModelRefAs(gn(spr), "sprite"));
          UI.resetSpriteLibrary();
        }
        break;
      case "deletepage":
      ...
      case "deletesound":
        ...
        getModelRefAs(gn(spr), "sprite").sounds = sounds;
        _Undo.redoScripts(data, page, spr);
        if (Palette.numcat == 3) {
          Palette.selectCategory(3);
        }
        break;
      case "recordsound":
        var recspr = getModelRefAs(gn(data[page][spr].id), "sprite");
  ...
  static copyPage(obj, page) {
    ...
    function nextStep2() {
      ...
      ScratchJr.stage.setPage(getModelRefAs(gn(obj.currentPage), "page"), false);
      _Undo.recreateAllScripts(obj);
      var spritename = obj[obj.currentPage].lastSprite;
      if (spritename && gn(spritename)) {
        var spr = getModelRefAs(gn(spritename), "sprite");
        var page2 = getModelRefAs(spr.div.parentNode, "page");
  ...
  static getPageOrder(data) {
    ...
    for (var i = 0; i < pages.length; i++) {
      res.push(getModelRefAs(gn(pages[i]), "page"));
    }
    return res;
  }
  ...
  static removePage(data, str) {
    ...
    if (ScratchJr.stage.pages.length == 0) {
      ...
      ScratchJr.stage.setViewPage(getModelRefAs(gn(data.currentPage), "page"));
      Thumbs.updateSprites();
      Thumbs.updatePages();
    }
  }
  ...
  static copySprite(data, page, spr) {
    ...
    Project.recreateObject(getModelRefAs(gn(page), "page"), spr, obj, fcn, data[page].lastSprite == spr);
  }
  static setSprite(page, data) {
    ...
    var pageobj = getModelRefAs(gn(page), "page");
    ...
    if (!lastsprite) {
      ...
      var cs = getModelRefAs(lastsprite, "sprite");
  ...
  static removeSprite(data, page, spr) {
    ...
    var sprite = getModelRefAs(gn(spr), "sprite");
...
var ScratchJr = class _ScratchJr {
  ...
  static unfocus(evt) {
    ...
    if (namedForms5.editable) {
      if (evt && evt.target == namedForms5.editable.field) {
        return;
      }
    }
    if (namedForms5.activetextbox) {
      if (evt && evt.target == namedForms5.activetextbox.typing) {
        return;
      }
    }
    if (namedForms5.projectname) {
      if (evt && evt.target == namedForms5.projectname.myproject) {
        return;
      }
    }
  ...
  static getSprite() {
    ...
    return getModelRefAs(gn(stage.currentPage.currentSpriteName), "sprite");
  }
  static gestureStart(e) {
    e.preventDefault();
    if (ScratchAudio.firstTime) {
  ...
  static flippage() {
    ...
    PlatformBridge.cleanassets("wav", function() {
      PlatformBridge.cleanassets("svg", function() {
        PlatformBridge.cleanassets("png", _ScratchJr.switchPage);
      });
    });
  }
  static switchPage() {
  ...
  static textClicked(e, div) {
    ...
    var ti = namedForms5.editable.field;
    ti.style.textAlign = "center";
    namedForms5.editable.style.left = dx + "px";
    var top = pt.y + 55 * scaleMultiplier;
    namedForms5.editable.style.top = top + "px";
    namedForms5.editable.className = "textform on";
  ...
  static handleTextFieldBlur(e) {
    ...
    focus.argValue = str;
    focus.setValue(str);
    namedForms5.editable.className = "textform off";
  ...
  static editDone() {
    ...
    if (activeFocus.isText()) {
      namedForms5.editable.field.blur();
    } else {
  ...
  static numEditDone() {
    ...
    ba.argValue = val;
    if (ba.daddy && hasModelRef(ba.daddy.div.parentNode)) {
...
var Project = class _Project {
  ...
  static thumbnailUnique(thumbnailMD5, projectID, callback) {
    ...
    IO.query(PlatformBridge.database, json, function(result) {
  ...
  static save(id, whenDone) {
      ...
      if (th && ScratchJr.editmode != "storyStarter") {
        ...
        if (thumb && thumb.md5.indexOf("samples/") < 0) {
          _Project.thumbnailUnique(thumb.md5, id, function(isUnique) {
            if (isUnique) {
              PlatformBridge.remove(thumb.md5, PlatformBridge.trace);
            }
          });
        }
      }
    ...
    function getMD5(dataurl) {
      ...
      PlatformBridge.getmd5(pngBase64, function(str) {
        if (!str) {
          resetSaving();
          return;
        }
    ...
    function savePNG(md5, pngBase64) {
      ...
      PlatformBridge.setmedianame(pngBase64, filename, "png", doNext);
    }
    function doNext(md5) {
      if (!metadata) {
        resetSaving();
  ...
  static encodeSprite(name2) {
    return getModelRefAs(gn(name2), "sprite").getData();
  }
  ...
  static drawSprites(page, scale, c, w, h, fcn) {
    ...
    function doNext(n) {
      if (!(n < page.div.childElementCount)) {
        ...
        var spr = getModelRefAs(page.div.childNodes[n], "sprite");
        if (!spr || !spr.shown) {
          doNext(n + 1);
        } else {
...
//# sourceMappingURL=chunk-EZU234IK.js.map
